...
const PAYMENT_METHODS = [
  ...
  { id: "card",   label: "Card Payment",      color: "bg-dark" },
];
...
function RecoverySuccessPage() {
  ...
  const [isFinderRole, setIsFinderRole] = useState(false);
  const particles = useRef<{ x: number; y: number; color: string; size: number; speed: number }[]>([]);
  ...
  useEffect(() => {
    particles.current = Array.from({ length: 60 }, () => ({
      x: Math.random() * 100,
      y: -10 - Math.random() * 20,
      color: ["#009A49", "#FCD116", "#CE1126", "#ffffff"][Math.floor(Math.random() * 4)],
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (profile) {
        setGuardianPoints(profile.guardian_points ?? 0);
      }
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -40 }} className="flex flex-col items-center max-w-md w-full">
            <div className="relative mb-8">
              <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 3 }}
                className="w-28 h-28 bg-primary/20 rounded-full flex items-center justify-center">
            ...
            <h1 className="text-5xl font-black font-clash uppercase tracking-tighter text-white mb-3 leading-none">
              Item <br /><span className="text-primary">Recovered!</span>
            </h1>
            <p className="text-white/40 max-w-sm font-bold uppercase tracking-widest text-[9px] leading-relaxed mb-6">
            ...
            <div className="flex gap-3 justify-center mb-4 w-full">
              <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-4">
                <p className="text-primary font-black text-3xl font-clash">+{pointsEarned}</p>
                <p className="text-white/30 text-[9px] font-black uppercase tracking-widest mt-1">Guardian Points</p>
              </div>
              <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-4">
                <p className="text-primary font-black text-3xl font-clash">+1</p>
                <p className="text-white/30 text-[9px] font-black uppercase tracking-widest mt-1">Recovery</p>
              </div>
            </div>
            ...
            <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 mb-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-white/30 text-xs font-bold">{guardianPoints} pts</p>
              </div>
              {guardian.next && (
                <div className="flex items-center gap-2">
                  <TrendingUp size={11} className="text-white/20" />
                  <p className="text-white/20 text-[9px] font-bold uppercase tracking-widest">{guardian.next}</p>
                </div>
              )}
            </div>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              ...
              className="w-full bg-primary text-dark py-5 rounded-[2rem] font-black tracking-[0.3em] text-sm shadow-[0_20px_50px_rgba(0,154,73,0.2)] flex items-center justify-center gap-3">
        ...
        {step === "rating" && (
          <motion.div key="rating" initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -60 }} className="flex flex-col items-center max-w-md w-full">
            ...
            <p className="text-white/40 text-sm font-bold uppercase tracking-widest mb-10">
              How was your experience with <span className="text-white">{otherUserName}</span>?
            </p>
            ...
            <div className="mb-10 h-6">
              {(hoverRating || rating) > 0 && (
                <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-white/60 font-black text-xs uppercase tracking-widest">
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              ...
              className="w-full bg-primary text-dark py-5 rounded-[2rem] font-black tracking-[0.3em] text-sm flex items-center justify-center gap-3 disabled:opacity-40">
              {submittingRating ? <><Loader2 size={18} className="animate-spin" /> Saving...</> : <>Submit Rating <ArrowRight size={18} /></>}
            </motion.button>
            <button onClick={() => setStep("tip")} className="mt-4 text-white/20 text-[10px] font-black uppercase tracking-widest hover:text-white/40 transition-colors">
              Skip for now
            </button>
          </motion.div>
        )}
        ...
        {step === "tip" && (
          <motion.div key="tip" initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -60 }} className="flex flex-col items-center max-w-md w-full">
            ...
            <p className="text-white/20 text-[10px] font-bold uppercase tracking-widest mb-8">Tips go to the Back2U platform to keep the service running</p>
            <div className="grid grid-cols-4 gap-3 w-full mb-5">
              {TIP_PRESETS.map(amount => (
                <motion.button key={amount} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  ...
                  className={`py-4 rounded-2xl border-2 transition-all font-black text-sm ${tipAmount === amount && !customTip ? "border-primary bg-primary/10 text-primary" : "border-white/10 text-white/40"}`}>
                  <span className="block text-[9px] font-bold uppercase tracking-widest mb-1 opacity-60">XAF</span>
                  {amount.toLocaleString()}
                </motion.button>
              ))}
            </div>
            <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center gap-3 mb-8 focus-within:border-primary transition-all">
            ...
            <div className="flex gap-3 w-full">
              <button onClick={() => setStep("done")} className="flex-1 py-5 border-2 border-white/5 rounded-[2rem] font-black text-xs tracking-widest text-white/30 hover:bg-white/5 transition-all">
                Skip
              </button>
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                onClick={() => { if (getFinalTipAmount()) setStep("payment"); }} disabled={!getFinalTipAmount()}
                className="flex-[2] py-5 bg-primary text-dark rounded-[2rem] font-black text-xs tracking-[0.2em] flex items-center justify-center gap-3 disabled:opacity-40">
        ...
        {step === "payment" && (
          <motion.div key="payment" initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -60 }} className="flex flex-col items-center max-w-md w-full">
            ...
            <p className="text-white/30 text-[10px] font-black uppercase tracking-widest mb-8">Choose your payment method</p>
            <div className="w-full space-y-3 mb-6">
              {PAYMENT_METHODS.map(method => (
                <motion.button key={method.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  ...
                  className={`w-full p-5 rounded-2xl border-2 flex items-center gap-4 transition-all text-left ${paymentMethod === method.id ? "border-primary bg-primary/5" : "border-white/10"} ${method.id === "card" ? "opacity-30 cursor-not-allowed" : ""}`}>
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <p className="text-white/20 text-[9px] font-bold uppercase tracking-widest">Secured by MeSomb</p>
            </div>
            <div className="flex gap-3 w-full">
        ...
        {step === "done" && (
          <motion.div key="done" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center max-w-md w-full">
            ...
            <h1 className="text-5xl font-black font-clash uppercase tracking-tighter text-white mb-4 leading-none">
              All <br /><span className="text-primary">Done!</span>
            </h1>
            <p className="text-white/40 max-w-sm font-bold uppercase tracking-widest text-[9px] leading-relaxed mb-8">
              Thank you for using Back2U. Your story helps build a more honest Cameroon.
            </p>
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              onClick={() => router.push("/dashboard")}
              className="w-full bg-white text-dark py-5 rounded-[2rem] font-black tracking-[0.3em] text-sm hover:bg-primary transition-all">
              Back to Dashboard
            </motion.button>
          </motion.div>
        )}
