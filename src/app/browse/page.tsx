    ...
    >
      ...
      <div className="aspect-[16/11] overflow-hidden relative bg-slate-100">
        {firstPhoto ? (
          <img
            src={firstPhoto}
            className={`w-full h-full object-cover transition-all duration-700 group-hover:scale-105 ${isSensitive ? "blur-xl scale-110" : "grayscale-[0.3] group-hover:grayscale-0"}`}
            alt={item.title}
          />
        ) : (
        ...
        {isPending && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <span className="px-4 py-2 bg-secondary text-dark rounded-full text-[9px] font-black uppercase tracking-widest">
              Pending Review
            </span>
          </div>
        )}
        ...
        {isSensitive && !isPending && (
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="px-4 py-2 bg-black/50 backdrop-blur-md text-white rounded-full text-[9px] font-black uppercase tracking-widest">
              Sensitive Item
            </span>
          </div>
        )}
        ...
        {/* Type badge */}
        <div className={`absolute top-5 left-5 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] shadow-xl ${isFound ? "bg-primary text-dark" : "bg-[#FF4D4D] text-white"}`}>
          {item.type}
        </div>
        ...
        <button
          ...
          className="absolute top-5 right-5 w-8 h-8 bg-black/40 backdrop-blur-md rounded-xl flex items-center justify-center text-white/60 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
        >
          <Flag size={12} />
        </button>
      </div>
      ...
      {/* Content */}
      <div className="p-8">
        <h3 className="text-xl font-black font-clash uppercase tracking-tight mb-4 group-hover:text-primary transition-colors text-dark">
          {item.title}
        </h3>
        <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-widest text-slate-400">
          <span className="flex items-center gap-1.5">
            ...
            {item.location_name || item.city || "Location unknown"}
          </span>
          <span>{timeAgo(item.created_at)}</span>
        </div>
        {/* Poster name — hidden if anonymous */}
...
export default function BrowseMarketplace() {
  ...
  const fetchItems = useCallback(async (pageNum: number, replace = false) => {
    ...
    try {
      ...
      // City-first sort: user's city items come first
      const sorted = userCity
        ? [
            ...newItems.filter(i => i.city === userCity),
            ...newItems.filter(i => i.city !== userCity),
          ]
        : newItems;
  ...
  useEffect(() => {
    ...
    setItems([]);
    fetchItems(0, true);
  }, [search, activeDateRange, typeFilter]);

  // Load more when page changes
  useEffect(() => {
    if (page > 0) fetchItems(page);
  }, [page]);
  ...
  // ── Map items (only items with location_name for now) ──
  const mapItems = items.filter(i => i.city);

  return (
    <main className="min-h-screen bg-white text-slate-900 overflow-x-hidden">

      {/* ── SUB-HEADER — unchanged layout ── */}
            ...
            <div className="flex bg-slate-100 p-1 rounded-2xl">
              {(["all", "lost", "found"] as const).map(t => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${typeFilter === t ? "bg-white shadow-sm text-black" : "text-slate-400"}`}
                >
                  {t}
                </button>
              ))}
          ...
          {view === "grid" && (
            ...
            >
              <header className="mb-10">
                <h1 className="text-5xl font-black font-clash tracking-tighter mb-6 italic text-dark uppercase">
                  Discovery Hub
                </h1>
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1 group">
                      ...
                      <input
                        ...
                        placeholder="Search items, locations, serial numbers..."
                        className="w-full pl-12 pr-6 py-4 bg-white border border-slate-100 rounded-[1.5rem] text-sm outline-none focus:ring-4 ring-primary/5 transition-all shadow-sm text-slate-900 placeholder:text-slate-400"
                      />
                    </div>
                    <Link
                      href="/report"
                      className="px-8 py-4 bg-dark text-white rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-primary hover:text-dark transition-all shadow-lg shadow-black/10"
                    >
              ...
              {/* Error */}
              {error && (
                <div className="flex items-center gap-3 bg-red-50 border border-red-100 rounded-2xl p-4 mb-8 text-red-500 text-sm font-bold">
                ...
                <>
                  {items.length === 0 ? (
                    <div className="text-center py-32">
                      ...
                      <Link href="/report" className="inline-flex mt-8 px-8 py-4 bg-dark text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-primary hover:text-dark transition-all">
                        Report an Item
                      </Link>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                      {items.map(item => (
                        <ItemCard key={item.id} item={item} onFlag={handleFlag} />
                      ))}
                    </div>
                  )}
        ...
        <AnimatePresence mode="wait">
          {view === "map" && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="relative w-full h-[calc(100vh-140px)]"
            >
              <Map
              ...
              >
                ...
                {mapItems.map(item => (
                  <Marker
                    key={item.id}
                    longitude={11.5021}  // placeholder — real coords come in Phase 4b with Mapbox geocoding
                    latitude={3.8480}
                    anchor="bottom"
                  ...
                  >
                    <motion.div
                      ...
                      className={`w-12 h-12 rounded-[1rem] shadow-2xl flex items-center justify-center border-2 cursor-pointer ${item.type === "found" ? "bg-white border-primary" : "bg-white border-[#FF4D4D]"}`}
                    >
                ...
                {selectedMapItem && (
                  ...
                  >
                    <div
                      className="p-4 bg-white cursor-pointer w-52"
                      onClick={() => window.location.href = `/browse/${selectedMapItem.id}`}
                    >
                      {selectedMapItem.photos?.[0] && (
                        <img
                          src={selectedMapItem.photos[0]}
                          className={`w-full h-28 object-cover rounded-xl mb-3 ${selectedMapItem.sensitivity !== "normal" ? "blur-md" : ""}`}
                          alt=""
                        />
                      )}
                      <div className={`inline-block px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest mb-2 ${selectedMapItem.type === "found" ? "bg-primary text-dark" : "bg-[#FF4D4D] text-white"}`}>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                        {selectedMapItem.location_name || selectedMapItem.city}
                      </p>
                    </div>
                  </Popup>
                )}
          ...
          >
            ...
            >
              ...
              <textarea
                ...
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm outline-none focus:ring-2 ring-red-200 resize-none mb-4"
              />
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-[2] py-4 bg-red-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-red-600 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
                >
                  {flagSubmitting ? <Loader2 size={14} className="animate-spin" /> : null}
                  Submit Flag
                </button>
              </div>
            </motion.div>
