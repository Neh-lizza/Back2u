    ...
    <main className="h-[calc(100vh-80px)] bg-white flex overflow-hidden mt-20">
      ...
      <aside className={`${showMobileChat ? "hidden md:flex" : "flex"} w-full md:w-[380px] flex-col border-r border-slate-100 shrink-0`}>
        <div className="p-6 border-b border-slate-100">
          ...
          <div className="relative group">
            ...
            <input type="text" value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search conversations..." className="w-full pl-12 pr-6 py-3 bg-slate-50 rounded-2xl text-sm outline-none focus:ring-2 ring-primary/20 transition-all text-slate-900 placeholder:text-slate-400" />
          </div>
        </div>

        <div className="flex-1 overflow-y-auto no-scrollbar">
          {loadingChats && <div className="flex items-center justify-center py-20"><Loader2 size={24} className="animate-spin text-primary" /></div>}
          ...
          {filteredChats.map(chat => (
            <button key={chat.id} onClick={() => openChat(chat)} className={`w-full p-5 flex gap-4 transition-all border-l-4 ${activeChat?.id === chat.id ? "bg-primary/5 border-primary" : "bg-white border-transparent hover:bg-slate-50"}`}>
              <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center font-black text-primary text-lg overflow-hidden shrink-0">
                {chat.other_user?.avatar_url ? <img src={chat.other_user.avatar_url} className="w-full h-full object-cover" alt="" /> : chat.other_user?.full_name?.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 text-left min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <h4 className="font-black text-sm text-dark truncate">{chat.other_user?.full_name ?? "Unknown"}</h4>
                  <span className="text-[10px] font-bold text-slate-400 uppercase shrink-0 ml-2">{chat.last_message ? timeAgo(chat.last_message.created_at) : timeAgo(chat.created_at)}</span>
                </div>
                <p className="text-xs text-slate-500 truncate mb-2">{chat.last_message?.type === "image" ? "📷 Photo" : chat.last_message?.content ?? "No messages yet"}</p>
                {chat.item && <span className="px-2 py-0.5 bg-slate-100 rounded text-[9px] font-black uppercase tracking-tighter text-slate-400">{chat.item.type === "lost" ? "🔴" : "🟢"} {chat.item.title}</span>}
              </div>
              {chat.unread_count > 0 && <div className="w-5 h-5 bg-primary rounded-full flex items-center justify-center text-[10px] font-black text-dark shrink-0">{chat.unread_count > 9 ? "9+" : chat.unread_count}</div>}
            </button>
          ))}
        </div>
      </aside>
          ...
          <>
            <header className="h-20 border-b border-slate-100 flex items-center justify-between px-8 bg-white/80 backdrop-blur-md z-10 shrink-0">
              <div className="flex items-center gap-4">
                <button onClick={() => setShowMobileChat(false)} className="md:hidden p-2 text-slate-400 hover:text-dark transition-colors"><ArrowLeft size={20} /></button>
                ...
                <div>
                  <h3 className="font-black text-sm uppercase tracking-tight text-dark">{activeChat.other_user?.full_name ?? "User"}</h3>
                  <p className="text-[10px] font-bold text-primary uppercase tracking-widest flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse" /> Re: {activeChat.item?.title}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {activeChat.status !== "recovered" && (
                  <button onClick={() => setShowRecoveryPrompt(true)} className="hidden md:flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-primary hover:text-dark transition-all">
                    <CheckCircle2 size={14} /> Item Recovered
                  </button>
                )}
                <button className="p-3 hover:bg-slate-100 rounded-xl text-slate-400 transition-colors"><MoreVertical size={18} /></button>
              </div>
            </header>

            <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-6 no-scrollbar">
              {loadingMessages && <div className="flex justify-center py-10"><Loader2 size={24} className="animate-spin text-primary" /></div>}
                ...
                if (isSystem) return (
                  <div key={msg.id} className="flex justify-center">
                    <div className="bg-primary/10 border border-primary/20 px-4 py-2 rounded-2xl flex items-center gap-3">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-primary">{msg.content}</p>
                    </div>
                  </div>
                );
                if (msg.type === "image") return (
                  <div key={msg.id} className={`flex gap-4 max-w-[70%] ${isMe ? "flex-row-reverse ml-auto" : ""}`}>
                    <img src={msg.content} className="w-56 h-56 object-cover rounded-[2rem] shadow-sm cursor-pointer hover:opacity-90 transition-opacity" alt="shared" onClick={() => window.open(msg.content, "_blank")} />
                  </div>
                );
                return (
                  <div key={msg.id} className={`flex gap-4 max-w-[85%] md:max-w-[70%] ${isMe ? "flex-row-reverse ml-auto" : ""}`}>
                    {!isMe && <div className="w-8 h-8 rounded-lg bg-slate-100 shrink-0 self-end overflow-hidden flex items-center justify-center text-[10px] font-black text-slate-400">{msg.sender?.full_name?.charAt(0).toUpperCase()}</div>}
                    {isMe ? (
                      <div className="bg-dark p-5 rounded-3xl rounded-br-none shadow-xl shadow-dark/10">
                        ...
                        <div className="flex justify-end items-center gap-1 mt-2">
                          <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">{formatTime(msg.created_at)}</p>
                      ...
                      <div className="bg-white border border-slate-100 p-5 rounded-3xl rounded-bl-none shadow-sm">
                        ...
                        <p className="text-[9px] font-bold text-slate-400 mt-2 uppercase tracking-widest">{formatTime(msg.created_at)}</p>
                      </div>
                    )}
                  </div>
                );
            ...
            <footer className="p-6 md:p-8 bg-white border-t border-slate-100 shrink-0">
              ...
              <div className="max-w-4xl mx-auto flex items-center gap-4 bg-slate-50 border border-slate-200 p-2 rounded-[2.5rem]">
                <button onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-400 hover:text-dark transition-colors"><Paperclip size={20} /></button>
                <input type="text" value={message} onChange={e => setMessage(e.target.value)} onKeyDown={handleKeyDown} placeholder={`Message ${activeChat.other_user?.full_name?.split(" ")[0] ?? ""}...`} className="flex-1 bg-transparent border-none outline-none text-sm font-medium px-2 text-dark placeholder:text-slate-400" />
                <button onClick={handleSend} disabled={!message.trim() || sending} className="bg-dark text-primary p-4 rounded-[2rem] hover:scale-105 active:scale-95 transition-all shadow-lg shadow-dark/20 disabled:opacity-40">
      ...
      <AnimatePresence>
        {showRecoveryPrompt && activeChat && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-6" onClick={() => setShowRecoveryPrompt(false)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={e => e.stopPropagation()} className="bg-white rounded-[3rem] p-10 max-w-sm w-full shadow-2xl text-center">
              <div className="w-20 h-20 bg-primary/10 rounded-[2rem] flex items-center justify-center mx-auto mb-6"><CheckCircle2 size={40} className="text-primary" /></div>
              <h3 className="font-clash font-black text-3xl uppercase tracking-tighter mb-3">Confirm Recovery</h3>
              <p className="text-slate-500 text-sm mb-8 leading-relaxed">Confirm that <span className="font-black text-dark">{activeChat.item?.title}</span> has been successfully returned. Both parties must confirm.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowRecoveryPrompt(false)} className="flex-1 py-4 border border-slate-100 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-400 hover:bg-slate-50 transition-all">Not Yet</button>
                <button onClick={handleConfirmRecovery} disabled={recovering} className="flex-[2] py-4 bg-primary text-dark rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-[1.02] transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                  {recovering ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} Yes, Confirmed!
                </button>
              </div>
            </motion.div>
          </motion.div>
