...
if (typeof window !== "undefined") {
  try {
    ...
    ({ BarChart, Bar, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } = r);
  } catch { /* recharts not installed yet */ }
}
...
export default function AdminDashboard() {
  ...
  const loadStats = async () => {
    ...
    const [
      ...
      db.from("items").select("category").not("category", "is", null),
      db.from("items").select("city").not("city", "is", null),
      db.from("items").select("type"),
      db.from("items").select("created_at").gte("created_at", new Date(Date.now() - 14 * 86400000).toISOString()),
    ]);

    const catCount: Record<string, number> = {};
    byCategory?.forEach((i: any) => { catCount[i.category] = (catCount[i.category] || 0) + 1; });
    const categoryData = Object.entries(catCount).sort((a,b) => b[1]-a[1]).slice(0,7).map(([name, count]) => ({ name: name.charAt(0).toUpperCase() + name.slice(1), count }));

    const cityCount: Record<string, number> = {};
    byCity?.forEach((i: any) => { cityCount[i.city] = (cityCount[i.city] || 0) + 1; });
    const cityData = Object.entries(cityCount).sort((a,b) => b[1]-a[1]).slice(0,8).map(([name, count]) => ({ name, count }));

    const lostCount  = byType?.filter((i: any) => i.type === "lost").length  ?? 0;
    const foundCount = byType?.filter((i: any) => i.type === "found").length ?? 0;
    ...
    recentDays?.forEach((i: any) => {
      const key = new Date(i.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" });
      if (key in dayCount) dayCount[key]++;
    });
    const dailyData = Object.entries(dayCount).map(([date, count]) => ({ date, count }));
    setAnalytics({ categoryData, cityData, lostCount, foundCount, dailyData });
    ...
    <main className="min-h-screen" style={{ background: "#F0F4F8" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 space-y-5">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            <p className="text-slate-400 text-base font-medium">Admin Portal</p>
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 leading-tight mt-0.5"
          ...
          <div className="flex items-center gap-3">
            <button onClick={loadAll} disabled={loading}
              className="w-10 h-10 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-primary hover:border-primary transition-all">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id as AdminTab)}
              ...
              style={{
                background: activeTab === tab.id ? "#222831" : "#fff",
                color: activeTab === tab.id ? "#fff" : "#94a3b8",
                border: `1px solid ${activeTab === tab.id ? "#222831" : "#e2e8f0"}`,
              }}>
              <tab.icon size={13} /> {tab.label}
              {(tab.badge ?? 0) > 0 && (
                <span className="w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-black"
                  style={{ background: activeTab === tab.id ? "#009A49" : "#CE1126", color: "#fff" }}>
                  {tab.badge}
                </span>
              )}
            </button>
        ...
        {activeTab === "overview" && stats && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {[
                { label: "Total Items",   value: stats.totalItems,                     bg: "#fce8e8", color: "#ef4444", icon: Package       },
                { label: "Active Items",  value: stats.activeItems,                    bg: "#fef0e6", color: "#f97316", icon: TrendingUp     },
                { label: "Total Users",   value: stats.totalUsers,                     bg: "#e8f9ee", color: "#22c55e", icon: Users          },
                { label: "Recoveries",    value: stats.totalRecoveries,                bg: "#f0fdfa", color: "#00ADB5", icon: CheckCircle2   },
                { label: "Tips (XAF)",    value: stats.totalTips.toLocaleString(),     bg: "#fef9c3", color: "#ca8a04", icon: DollarSign     },
              ].map((s, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  className="rounded-2xl p-4 flex items-center gap-3"
                  style={{ background: s.bg }}>
                  <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0" style={{ background: s.color }}>
                    <s.icon size={20} className="text-white" />
                  </div>
                  <div>
                    <p className="font-black text-slate-900 leading-none mb-0.5"
                      style={{ fontSize: "1.4rem", fontFamily: "'Clash Grotesk', sans-serif" }}>{s.value}</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: "Pending Review", value: stats.pendingReview, icon: ShieldAlert, bg: "#fef9c3", color: "#ca8a04", tab: "pending" as AdminTab },
                { label: "Flagged Items",  value: stats.pendingFlags,  icon: Flag,        bg: "#fce8e8", color: "#ef4444", tab: "flags"   as AdminTab },
                { label: "Banned Users",   value: stats.bannedUsers,   icon: ShieldX,     bg: "#fce8e8", color: "#CE1126", tab: "fraud"   as AdminTab },
                { label: "Flagged Users",  value: stats.flaggedUsers,  icon: UserX,       bg: "#fef0e6", color: "#f97316", tab: "fraud"   as AdminTab },
              ].map((s, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 + i * 0.05 }}
                  onClick={() => setActiveTab(s.tab)}
                  className="rounded-2xl p-4 flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity"
                  style={{ background: s.bg }}>
                  <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0" style={{ background: s.color }}>
                    <s.icon size={20} className="text-white" />
                  </div>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {[
                ...
                <motion.div key={i} whileHover={{ y: -2 }} onClick={() => setActiveTab(card.tab)}
                  className="rounded-2xl p-4 cursor-pointer transition-all hover:opacity-90"
        ...
        {activeTab === "pending" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            {pendingItems.length === 0 ? (
              <div className="rounded-2xl border border-amber-100 flex flex-col items-center py-16 gap-3"
                style={{ background: "#fffbeb" }}>
                <ShieldCheck size={36} className="text-amber-400" />
                <p className="font-bold text-amber-300 text-sm">All clear - no items pending review</p>
              </div>
            ) : pendingItems.map(item => (
              <motion.div key={item.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                className="rounded-2xl border border-amber-100 overflow-hidden" style={{ background: "#fffcf0" }}>
        ...
        {activeTab === "flags" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            {flaggedItems.length === 0 ? (
              <div className="rounded-2xl border border-red-100 flex flex-col items-center py-16 gap-3"
                style={{ background: "#fff5f5" }}>
                <ShieldCheck size={36} className="text-red-300" />
                <p className="font-bold text-red-200 text-sm">No flagged items</p>
              </div>
            ) : flaggedItems.map(item => (
              <motion.div key={item.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                className="rounded-2xl border border-red-100 overflow-hidden" style={{ background: "#fff5f5" }}>
                <div className="flex gap-4 p-4">
                  <div className="w-20 h-20 rounded-xl overflow-hidden bg-slate-100 shrink-0">
                    {item.photos?.[0]
        ...
        {activeTab === "fraud" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-red-100 p-4" style={{ background: "#fff5f5" }}>
                <div className="flex items-center gap-3">
              ...
              <div className="rounded-2xl border border-orange-100 p-4" style={{ background: "#fff7ed" }}>
                <div className="flex items-center gap-3">
            ...
            {users.filter(u => u.is_flagged || u.is_banned).length > 0 && (
              <div className="rounded-2xl overflow-hidden" style={{ background: "#fff5f5", border: "1px solid #fecaca" }}>
                <div className="px-4 py-3" style={{ borderBottom: "1px solid #fecaca" }}>
                  <h3 className="font-bold text-xs text-red-700">Flagged & Banned Users</h3>
                </div>
                {users.filter(u => u.is_flagged || u.is_banned).map((u, i) => (
                  <div key={u.id} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? "border-t border-slate-50" : ""}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-xs text-purple-900 truncate">{u.full_name}</p>
        ...
        {activeTab === "facebook" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
            <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background: "#eff6ff", border: "1px solid #bfdbfe" }}>
            ...
            {fbShareRequests.filter(r => r.status === "pending").map(req => (
              <motion.div key={req.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                className="rounded-2xl overflow-hidden" style={{ background: "#f0f7ff", border: "1px solid #bfdbfe" }}>
                <div className="flex gap-4 p-4">
        ...
        {activeTab === "users" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            {users.map((u, i) => (
              <div key={u.id} className={`flex items-center gap-3 px-4 py-3 hover:bg-purple-100 transition-all ${i > 0 ? "border-t border-purple-100" : ""}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] text-slate-400 font-medium">{u.city ?? "No city"} · {u.items_count} reports · {u.rating?.toFixed(1) ?? "0.0"} stars</p>
                </div>
                <div className="flex gap-1.5 shrink-0">
                  {u.role !== "admin" && (
        ...
        {activeTab === "archived" && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl overflow-hidden" style={{ background: "#f8fafc", border: "1px solid #e2e8f0" }}>
            {archivedItems.length === 0 ? (
              ...
              <div key={item.id} className={`flex items-center gap-3 px-4 py-3 hover:bg-purple-100 transition-all ${i > 0 ? "border-t border-purple-100" : ""}`}>
        ...
        {activeTab === "analytics" && !ResponsiveContainer && (
          <div className="rounded-2xl p-8 text-center" style={{ background: "#fff", border: "1px solid #e2e8f0" }}>
            <p className="font-bold text-slate-400 text-sm mb-2">Charts require recharts</p>
            <code className="text-xs text-slate-300 bg-slate-50 px-3 py-1.5 rounded-lg">npm install recharts</code>
          </div>
        )}
        {activeTab === "analytics" && ResponsiveContainer && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Total Reports",  value: stats?.totalItems ?? 0,      color: "#009A49" },
                { label: "Recovered",      value: stats?.totalRecoveries ?? 0,  color: "#FCD116" },
                { label: "Active",         value: stats?.activeItems ?? 0,      color: "#3b82f6" },
                { label: "Recovery Rate",  value: stats?.totalItems ? Math.round(((stats?.totalRecoveries ?? 0) / stats.totalItems) * 100) + "%" : "0%", color: "#009A49" },
              ].map((s, i) => (
                <div key={i} className="rounded-2xl p-4" style={{ background: "#fff", border: "1px solid #e2e8f0", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
              ...
              <div className="flex items-center gap-8 justify-center">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-bold text-slate-600">Recovered - {stats?.totalRecoveries ?? 0} items</p>
                  </div>
                </div>
              </div>
            </div>
